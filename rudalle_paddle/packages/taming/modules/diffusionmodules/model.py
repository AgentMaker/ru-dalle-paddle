# -*- coding: utf-8 -*-
# pypaddle_diffusion + derived encoder decoder
import math
import paddle
...
def get_timestep_embedding(timesteps, embedding_dim):
    ...
    emb = timesteps.astype(paddle.float32)[:, None] * emb[None, :]
        ...
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks+1):
                h = self.up[i_level].block[i_block](
                    paddle.concat([h, hs.pop()], axis=1), temb)
...
class Decoder(nn.Layer):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 8), num_res_blocks,
        ...
        # z to block_in
        self.conv_in = paddle.nn.Conv2D(z_channels,
                                        block_in,
                                        kernel_size=3,
                                        stride=1,
                                        padding=1)
...
class VUNet(nn.Layer):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 8), num_res_blocks,
        ...
        # downsampling
        self.conv_in = paddle.nn.Conv2D(c_channels,
                                        self.ch,
                                        kernel_size=3,
                                        stride=1,
                                        padding=1)
        ...
        self.z_in = paddle.nn.Conv2D(z_channels,
                                     block_in,
                                     kernel_size=1,
                                     stride=1,
                                     padding=0)
...
class SimpleDecoder(nn.Layer):
    def __init__(self, in_channels, out_channels, *args, **kwargs):
        super().__init__()
        self.model = nn.LayerList([nn.Conv2D(in_channels, in_channels, 1),
        ...
        self.conv_out = paddle.nn.Conv2D(in_channels,
                                         out_channels,
                                         kernel_size=3,
                                         stride=1,
                                         padding=1)
...
class UpsampleDecoder(nn.Layer):
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks + 1):
                res_block.append(ResnetBlock(in_channels=block_in,
                                             ...
                                             dropout=dropout))
                block_in = block_out
            self.res_blocks.append(nn.LayerList(res_block))
            if i_level != self.num_resolutions - 1:
